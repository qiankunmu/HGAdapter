...
from transformers import CodeLlamaTokenizer, LlamaTokenizer, GenerationConfig
import numpy as np
...
from MyDataset import CodeSummarizationCSNLlamaDataset, CodeSummarizationCSNLlamaCollater
import sys

sys.path.append("..")
from models.HGAdapterConfig import HGAdapterConfig
from models.LlamaHGAdapterModels import MyLlamaHGAdapterForCausalLM
...
def main():
    ...
    tokenizer = LlamaTokenizer.from_pretrained(pretrain_model_name_or_path)
    ...
    for language in languages:
        ...
        collate_fn = CodeSummarizationCSNLlamaCollater(tokenizer)
        collate_fn2 = CodeSummarizationCSNLlamaCollater(tokenizer, "valid")

        train_dataset = CodeSummarizationCSNLlamaDataset(train_data_path, train_src_data_path, tokenizer, parser,
        ...
        valid_dataset = CodeSummarizationCSNLlamaDataset(valid_data_path, valid_src_data_path, tokenizer, parser,
                                                         max_seq_len=max_seq_len, max_tgt_len=max_tgt_len,
                                                         overwrite=False, mode="valid")
        valid_dataloader = DataLoader(valid_dataset, eval_batch_size, False, collate_fn=collate_fn2)
        test_dataset = CodeSummarizationCSNLlamaDataset(test_data_path, test_src_data_path, tokenizer, parser,
                                                        max_seq_len=max_seq_len, max_tgt_len=max_tgt_len,
