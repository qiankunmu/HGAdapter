...
from transformers import add_start_docstrings, LlamaPreTrainedModel, LlamaConfig
...
from transformers.models.llama.modeling_llama import LlamaDecoderLayer, LLAMA_START_DOCSTRING, LlamaRMSNorm, \
    LLAMA_INPUTS_DOCSTRING, LlamaFlashAttention2, LlamaMLP, _CONFIG_FOR_DOC
from transformers.utils import add_start_docstrings_to_model_forward, logging, replace_return_docstrings

from .LlamaHyperStructAdaptersMixins import LlamaDecoderLayerHyperStructAdaptersMixin

# copied from adapter.models.llama.modeling_llama and transformers.models.modeling_llama
logger = logging.get_logger(__name__)


class LlamaDecoderHyperStructAdapterLayer(LlamaDecoderLayer, LlamaDecoderLayerHyperStructAdaptersMixin):
    ...
    def forward(
        ...
        hidden_states = self.attention_adapters(hidden_states, residual, None, hyperedge_indexs=hyperedge_indexs,
                                                edge_types=edge_types, **kwargs)
        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        hidden_states = self.mlp(hidden_states)
        hidden_states = self.output_adapters(hidden_states, residual, None, hyperedge_indexs=hyperedge_indexs,
                                             edge_types=edge_types, **kwargs)
...
class LlamaHyperStructAdapterModel(LlamaModelAdapterMixin, LlamaPreTrainedModel):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderHyperStructAdapterLayer(config) for _ in range(config.num_hidden_layers)])
...
class MyLlamaHyperStructAdapterModel(EmbeddingAdaptersWrapperMixin, ModelWithFlexibleHeadsAdaptersMixin,
                                     LlamaPreTrainedModel):
    _tied_weights_keys = []  # needs to be empty since LLaMA does not yet support prompt tuning

    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaHyperStructAdapterModel(config)
...
class MyLlamaHyperStructAdapterForCausalLM(LlamaPreTrainedModel):
