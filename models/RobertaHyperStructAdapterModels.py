...
class RobertaHyperStructAdapterSelfOutput(BertSelfOutputHyperStructAdaptersMixin):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor, hyperedge_indexs=None,
                edge_types=None, layer_norm=None) -> torch.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.adapter_layer_forward(hidden_states, input_tensor, self.LayerNorm, hyperedge_indexs,
                                                   edge_types)
        return hidden_states


# Copied from transformers.models.bert.modeling_bert.BertOutput
class RobertaHyperStructAdapterOutput(BertOutputHyperStructAdaptersMixin):
    def __init__(self, config):
        super().__init__()
...
class RobertaHyperStructAdapterAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = RobertaSelfAttentionWithAdapters(
            config, position_embedding_type=position_embedding_type
        )
        self.output = RobertaHyperStructAdapterSelfOutput(config)
...
class RobertaHyperStructAdapterLayer(BertLayerAdaptersMixin, nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = RobertaHyperStructAdapterAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = RobertaHyperStructAdapterAttention(config, position_embedding_type="absolute")
        self.intermediate = RobertaIntermediate(config)
        self.output = RobertaHyperStructAdapterOutput(config)
    ...
    def forward(
        ...
        layer_output = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output, hyperedge_indexs,
            edge_types
        )
    ...
    def feed_forward_chunk(self, attention_output, hyperedge_indexs, edge_types):
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output, hyperedge_indexs, edge_types)
        return layer_output


# Copied from transformers.models.bert.modeling_bert.BertEncoder with Bert->Roberta
class RobertaHyperStructAdapterEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RobertaHyperStructAdapterLayer(config) for _ in range(config.num_hidden_layers)])
...
class RobertaHyperStructAdapterModel(BertModelAdaptersMixin, RobertaPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = RobertaHyperStructAdapterEncoder(config)
...
class MyRobertaHyperStructAdapterModel(EmbeddingAdaptersWrapperMixin, ModelWithFlexibleHeadsAdaptersMixin,
                                       RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.roberta = RobertaHyperStructAdapterModel(config)
