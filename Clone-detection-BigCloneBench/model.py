...
class CloneDetectionBCBModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, struct_mask_input=None):
        output = self.model(input_ids, attention_mask, struct_mask_input=struct_mask_input).last_hidden_state
...
class CloneDetectionBCBCodellamaModel(nn.Module):
    def __init__(self, model, config):
        super().__init__()

        self.model = model

        self.feed_forward = nn.Linear(config.hidden_size * 2, 768)
